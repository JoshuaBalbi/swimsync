...
function PracticeInfoForm({
  onCreatePractice,
  mode = "text",
  externalGeneratedPractice = null,
  saving = false,
}) {
  ...
  useEffect(() => {
    if (!externalGeneratedPractice) return;

    setFormData((current) => ({
      ...current,
      type: externalGeneratedPractice.type || current.type,
      focus: externalGeneratedPractice.focus || current.focus,
      totalAmount: externalGeneratedPractice.totalAmount || current.totalAmount,
      workoutText: externalGeneratedPractice.workoutText || current.workoutText,
      notes: externalGeneratedPractice.notes || current.notes,
    }));
  }, [externalGeneratedPractice]);
  ...
  function handleSubmit(e) {
    ...
    if ((mode === "text" || mode === "ai") && !formData.workoutText) {
      alert("Please type or generate a practice.");
      return;
    }

    if (mode === "builder" && !generatedWorkout) {
    ...
    const finalData = {
      ...formData,
      workoutText: mode === "builder" ? generatedWorkout : formData.workoutText,
      totalAmount:
        mode === "builder" && calculatedYardage > 0
  ...
  return (
    <div className="card shadow-sm border-0 rounded-4 mb-4">
      <div className="card-body p-4">
        <h3 className="fw-bold text-primary mb-3">Practice Details</h3>
        ...
        <form onSubmit={handleSubmit}>
          <div className="row g-3">
            ...
            {mode === "ai" && (
              <div className="col-12">
                <label className="form-label">AI Generated Practice</label>
                <textarea
                  name="workoutText"
                  className="form-control"
                  rows="10"
                  value={formData.workoutText || ""}
                  onChange={handleChange}
                  placeholder="Generate a practice above, then click Use This Practice."
                  required
                />
                <div className="form-text">
                  You can edit the AI-generated practice before saving it.
                </div>
              </div>
            )}
          ...
          <div className="form-check mt-3">
            ...
            <div className="form-text">
              Public practices can be viewed by independent swimmers. Private
              practices are for your team.
            </div>
          </div>
